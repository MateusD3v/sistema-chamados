...
export default function TicketList({ filter = 'ALL', setFilter, refreshTrigger, onUpdate }) {
  ...
  useEffect(() => {
    // Função de rolagem ultra-robusta para Chrome e outros navegadores
    ...
    // Tenta resetar o scroll imediatamente e em intervalos para vencer o Chrome
    forceScrollTop();
    const times = [0, 50, 100, 200, 500];
    const timers = times.map(ms => setTimeout(forceScrollTop, ms));
  ...
  return (
    <>
      <section className="card card-elev ws-center" style={{ height: '100%' }}>
